...
const BottomNav = () => {
  ...
  const tabs = [
    {
      ...
      icon: (
        <svg
          ...
          height="22"
          viewBox="0 0 24 24"
          fill={filter === "All" ? "currentColor" : "none"}
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
    ...
    },
    {
      ...
      icon: (
        <svg
          ...
          height="22"
          viewBox="0 0 24 24"
          fill={filter === "Important" ? "currentColor" : "none"}
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full bg-white/20 dark:bg-black/20 backdrop-blur-[40px] z-40 md:hidden flex justify-between items-center px-1 sm:px-3 pt-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] sm:pt-2 sm:pb-[calc(0.75rem+env(safe-area-inset-bottom))] border-t border-white/50 dark:border-white/10 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] rounded-t-[32px]">
      {" "}
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            <div
              className={`mb-1 transition-transform duration-300 ${isActive ? "scale-110 drop-shadow-md" : ""}`}
            >
              {tab.icon}
            </div>
            <span
              className={`text-[10px] sm:text-xs font-semibold transition-all duration-300 ${isActive ? "opacity-100 font-bold" : "opacity-70"}`}
            >
              {tab.label}
            </span>
            {/* Trash Badge */}
            ...
            {isActive && (
              <div className="h-1.5 w-1.5 rounded-full bg-black dark:bg-white absolute -bottom-2"></div>
            )}
          </button>
        );
      })}
