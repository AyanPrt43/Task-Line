...
function App() {
  ...
  const addTask = async (newTask) => {
    ...
    const taskData = { important: false, date: today, deleted: false, ...newTask };
    
    // Optimistic update
    const tempId = Date.now().toString();
    setTask((prev) => [{ id: tempId, _id: tempId, ...taskData }, ...prev]);
    
    try {
      const res = await axios.post(`http://${window.location.hostname}:5001/api/tasks`, taskData, await getHeaders());
      // Replace temp task with real task from DB
      setTask((prev) => prev.map(t => t.id === tempId ? { ...res.data, id: res.data._id } : t));
    } catch (err) {
  ...
  const updateTask = async (id, updatedTask) => {
    setTask((prev) => prev.map((t) => (t.id === id || t._id === id ? updatedTask : t)));
    try {
      await axios.put(`http://${window.location.hostname}:5001/api/tasks/${updatedTask._id || id}`, updatedTask, await getHeaders());
    } catch (err) {
  ...
  const deleteTask = async (id) => {
    setTask((prev) => prev.map((t) => (t.id === id || t._id === id ? { ...t, deleted: true } : t)));
    try {
      const t = task.find(t => t.id === id || t._id === id);
      await axios.put(`http://${window.location.hostname}:5001/api/tasks/${t._id || id}`, { ...t, deleted: true }, await getHeaders());
    } catch (err) {
  ...
  const permanentDeleteTask = async (id) => {
    ...
    try {
      await axios.delete(`http://${window.location.hostname}:5001/api/tasks/${id}`, await getHeaders());
    } catch (err) {
  ...
  const restoreTask = async (id) => {
    setTask((prev) => prev.map((t) => (t.id === id || t._id === id ? { ...t, deleted: false } : t)));
    try {
      const t = task.find(t => t.id === id || t._id === id);
      await axios.put(`http://${window.location.hostname}:5001/api/tasks/${t._id || id}`, { ...t, deleted: false }, await getHeaders());
    } catch (err) {
  ...
  const toggleCompleted = async (id) => {
    ...
    setTask((prev) => prev.map((t) => (t.id === id || t._id === id ? updated : t)));
    try {
      await axios.put(`http://${window.location.hostname}:5001/api/tasks/${t._id || id}`, updated, await getHeaders());
    } catch (err) {
  ...
  const toggleImportant = async (id) => {
    const t = task.find(t => t.id === id || t._id === id);
    const updated = { ...t, important: !t.important };
    setTask((prev) => prev.map((t) => (t.id === id || t._id === id ? updated : t)));
    try {
      await axios.put(`http://${window.location.hostname}:5001/api/tasks/${t._id || id}`, updated, await getHeaders());
    } catch (err) {
  ...
  const getFilteredTasks = () => {
    ...
    let filtered = task.filter((t) => {
      // Search filter — applies across all views
      if (searchQuery.trim()) {
      ...
      // Trash view — only show deleted tasks
      if (filter === "Trash") return t.deleted === true;

      // All other views — hide deleted tasks
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        // User logged in, fetch their tasks and profile from MongoDB
        try {
          const token = await user.getIdToken();
          
          // Fetch Profile
          const profileRes = await axios.get(`http://${window.location.hostname}:5001/api/users/me`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          setUserProfile(profileRes.data);

          // Fetch Tasks
          const res = await axios.get(`http://${window.location.hostname}:5001/api/tasks`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          // Map _id to id for local consistency
          const tasksFromServer = res.data.map(t => ({ ...t, id: t._id }));
  ...
  return (
    ...
    >
      <div className="h-full w-full flex flex-col md:flex-row relative overflow-hidden bg-gradient-to-br from-[#c8b3a6] via-[#aabdb5] to-[#749f99] dark:from-[#3a3532] dark:via-[#2f3d37] dark:to-[#223d38] pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
        <div className={`
        ...
        `}>
          <div className="w-full h-full relative">
            ...
            />
            {/* Collapse Toggle Arrow (Desktop) - rectangular edge tab */}
            <button
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className={`transform transition-transform duration-300 ${sidebarOpen ? '' : 'rotate-180'}`}>
          ...
          <div className="flex-1 min-h-0 w-full bg-white/10 dark:bg-black/20 p-3 pb-[100px] sm:p-5 sm:pb-[100px] md:pb-5 overflow-y-auto scrollbar-glass">
            ...
            {filteredTasks.length === 0 && filter !== "Trash" && (
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <h3 className="text-2xl font-bold text-black/70 dark:text-white/70 mb-2">No tasks found</h3>
                <p className="text-base font-medium text-black/50 dark:text-white/50">Add a task to get started!</p>
              </div>
            )}
            {filteredTasks.map((t) => (
              <NewTasks key={t.id || t._id} task={t} isTrashView={filter === "Trash"} />
            ))}
          </div>
        </div>
        <BottomNav />
       ...
       currentUser.providerData.some(p => p.providerId === 'password') && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-md p-4">
          <div className="bg-white/10 dark:bg-black/60 backdrop-blur-2xl rounded-3xl border border-white/20 dark:border-white/10 shadow-2xl p-8 relative overflow-hidden text-center max-w-sm">
            <h2 className="text-3xl font-bold text-black dark:text-white mb-4">Verify Your Email</h2>
            <p className="text-black/60 dark:text-white/60 mb-6">
              We've sent a verification link to your email address. Please click it to continue to <b>Ayan's Task Manager</b>.
            </p>
            <button 
